...
describe('Dashboard Administrador - ARPA', () => {
  ...
  // ── Prueba 6: Registrar estudiante con email duplicado de tutor ────────────
  // Prueba extra de valor: valida manejo de errores del back en el flujo completo.
  describe('6. Registrar estudiante con datos que generan conflicto', () => {
    beforeEach(() => {
      cy.loginAsAdmin();
      cy.intercept('GET', 'http://localhost:3000/api/students').as('getStudents');
      cy.intercept('GET', 'http://localhost:3000/api/tutors').as('getTutors');
    ...
    it('muestra mensaje de error del back cuando el student_login_id ya existe tras 3 reintentos', () => {
      // Registramos al mismo estudiante dos veces para agotar los reintentos.
      // El back intenta 3 veces generar un ID único antes de fallar.
      // Esta prueba crea la condición de fallo controladamente.

      cy.intercept('POST', 'http://localhost:3000/api/students').as('createStudent');

      // Primera creación — debe exitosa.
      cy.get('.btn-add-primary').contains('Añadir estudiante').click();
      cy.get('.modal-box').within(() => {
        cy.get('input[placeholder="Nombre"]').type('Duplicado');
        cy.get('input[placeholder="Apellido"]').type('Prueba');
      ...
      // Interceptamos el POST para verificar que el back maneja duplicados correctamente.
      // El back genera duplicadoprueba1, duplicadoprueba2... hasta 3 intentos.
      // En pruebas manuales previas ya existe duplicadoprueba1 de la prueba 1.
      // Esta prueba documenta el comportamiento del sistema ante colisiones.
      cy.get('.btn-add-primary').contains('Añadir estudiante').click();
      cy.get('.modal-box').within(() => {
        cy.get('input[placeholder="Nombre"]').type('Duplicado');
        cy.get('input[placeholder="Apellido"]').type('Prueba');
        cy.get('select').first().select('B2');
        cy.get('select').last().find('option').not('[value=""]').first().then(($opt) => {
          cy.get('select').last().select($opt.val());
        });
        cy.get('.btn-add-primary').click();
      ...
      // El segundo registro debe ser exitoso también — el back incrementa el sufijo.
      cy.wait('@createStudent').then(({ response }) => {
        expect(response.statusCode).to.eq(201);
        // El ID generado debe ser duplicadoprueba2.
        expect(response.body.student.student_login_id).to.match(/^duplicadoprueba\d+$/);
        expect(response.body.student.student_login_id).not.to.eq('duplicadoprueba1');
      });

      cy.get('.modal-box').should('not.exist');
      // Ambas cards deben estar visibles en la lista.
      cy.get('.card-student-name').contains('Duplicado Prueba').should('have.length.at.least', 1);
    });
  });
